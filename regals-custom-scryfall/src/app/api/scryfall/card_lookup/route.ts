...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    const { name, set, cn } = await request.json();
    if (!name || !set || !cn) {
    ...
    const fetchByNameAndSet = async () => {
      ...
      return data.data.find((card: { name: string; collector_number: string }) => (
        card.name === cardName && card.collector_number === collectorNumber
    ...
    return NextResponse.json({ error: `${cardName} was not found on Scryfall` }, { status: 404 });
  } catch (err) {
